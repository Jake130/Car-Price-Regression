...
class Argument:
    ...
    3: batch size
    4: learning rate
    5: SGD momentum (non-stochastic AlanW if -1)
    6: Use ReLU (Sigmoid if False)
    """
    ...
    def __init__(self, values: list):
        ...
        self.name = "{}_{}x{}_bs{}_lr{}_sg{}_rl{}".format(
            self.generic_name,
            self.depth,
            self.width,
            self.batch_size,
...
class Model(nn.Module):
    ...
    def __init__(self, args: Argument):
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Linear(48, args.width), # 48 because 48 features
            torch.nn.ReLU() if args.use_relu else torch.nn.Sigmoid(),
            *[torch.nn.Linear(args.width, args.width) if (i%2==0) else torch.nn.ReLU() if args.use_relu else torch.nn.Sigmoid() for i in range(args.depth*2)],
            torch.nn.Linear(args.width, 1)
        )
